<template>
    <div class="forum-container min-h-[calc(100vh-72px)] bg-gray-50 grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-0">
        <div class="sidebar-left overflow-y-auto p-4 border-r border-gray-200">
            <ForumSidebarLeft :user-id="userId" :selected-community="selectedCommunity"
                @community-selected="selectedCommunity = $event" />
        </div>
        <div class="main-content overflow-y-auto p-4 h-full min-h-0">
            <ForumMainContent v-if="selectedCommunity" :posts="filteredPosts" :community="selectedCommunity"
                :userId="userId" />
            <div v-else>
